...
function RoomList() {
  ...
  useEffect(() => {
    ...
    const fetchRooms = async () => {
      try {
        const response = await fetch('http://localhost:8001/api/admin/getallrooms', {
          method: "GET",
          headers: {
  ...
  //   try {
  //     const response = await fetch(`http://localhost:8001/api/admin/deleteroom/${roomId}`, {
  //       method: "DELETE",
  //       headers: {
  //         "Content-Type": "application/json",
  //         authtoken: `${authtoken}`,
          ...
          {rooms.map((room) => (
            <tr key={room._id}>
              ...
              {/* <td className="border px-4 py-2">
                <button onClick={() => handleDeleteRoom(room._id)} className="text-red-500">
                  Delete
                </button>
              </td> */}
            </tr>
